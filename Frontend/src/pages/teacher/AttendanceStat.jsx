  ...
  const fetchAttendanceStats = async () => {
    try {
      ...
      const [statsRes, absentRes] = await Promise.all([
        axios.get(
          `/teacher/attendance/stats?startDate=${startDate}&endDate=${endDate}&minAbsences=${
            minAbsences || 0
          }`
        ),
        axios.get(`/teacher/attendance/absent?threshold=${minAbsences || 1}&month=${month}&year=${year}`)
      ]);
