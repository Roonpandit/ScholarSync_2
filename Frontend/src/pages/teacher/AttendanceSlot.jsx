...
const AttendanceSlot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setCreatingSlot(false); // Reset loading state
    }
  };
  ...
  const handleApproveAttendance = async (attendanceId) => {
    try {
      ...
      await axios.post(`/teacher/attendance/${attendanceId}/approve`, {}, config);
  ...
  const handleRejectAttendance = async (attendanceId) => {
    ...
    try {
      ...
      await axios.post(`/teacher/attendance/${attendanceId}/reject`, { remark: remark.trim() }, config);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {slots.length > 0
              ? `Showing ${slots.length} attendance slot${slots.length !== 1 ? "s" : ""
              }`
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className={`inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white ${showAddForm
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {(() => {
                      ...
                      return filteredSlots.map((slot) => (
                        <tr
                          key={slot._id}
                          className={`hover:bg-gray-50 ${slot.isExpired ? "opacity-70" : ""
                            }`}
                        >
...
`}
      ...
      <Modal
        isOpen={showAddForm}
        onClose={() => !creatingSlot && setShowAddForm(false)}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6">
              <button
                ...
                disabled={creatingSlot}
                className={`w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${creatingSlot
              ...
              <button
                type="submit"
                disabled={creatingSlot}
                className={`w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 ${creatingSlot
      ...
      <Modal
        ...
        onClose={() => !deletingSlot && setShowDeleteModal(false)}
        ...
        <div className="space-y-4">
              ...
              {slotToDelete && (
                <div className="mt-3 p-3 bg-gray-50 rounded-md">
                  <div className="text-sm text-gray-700">
                    <strong>Date:</strong> {new Date(slotToDelete.date).toLocaleDateString('en-US', {
                      weekday: 'long',
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                  ...
                  <div className="text-sm text-gray-700">
                    <strong>Time:</strong> {new Date(slotToDelete.startTime).toLocaleTimeString('en-US', {
                      hour: 'numeric',
                      minute: 'numeric',
                      hour12: false,
                      timeZone: 'Asia/Kolkata'
          ...
          <div className="flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6">
            <button
              ...
              className={`w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 ${deletingSlot
                ? 'text-gray-400 bg-gray-100 cursor-not-allowed'
                : 'text-gray-700 bg-white hover:bg-gray-50'
                }`}
            >
            ...
            <button
              ...
              onClick={async () => {
                if (slotToDelete?._id) {
                  await handleDeleteSlot(slotToDelete._id);
                }
              }}
              disabled={deletingSlot}
              className={`w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all duration-200 ${deletingSlot
