...
import { Calendar, ChevronRight, BarChart, ArrowRight, AlertCircle } from 'lucide-react';
...
const AttendanceStats = () => {
  ...
  const fetchAttendanceStats = async () => {
    try {
      ...
      const res = await axios.get(`/admin/attendance/stats?month=${month}&year=${year}&minAbsences=${minAbsences || 0}`);
  ...
  const safeStats = Array.isArray(stats) ? stats.filter(stat => stat && stat.student) : [];
  ...
  const renderMobileStudentCard = (stat) => {
    ...
    return (
      <div key={student._id || Math.random()} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        <div 
    ...
    <div className="bg-white rounded-lg shadow-sm p-4 md:p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Student attendance summary</p>
        </div>
      ...
      <div className="bg-gray-50 rounded-lg p-3 md:p-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
          ...
          <div className="flex flex-col">
            <label htmlFor="minAbsences" className="text-xs text-gray-500 mb-1">Minimum Absences</label>
            <input
              type="number"
              id="minAbsences"
              name="minAbsences"
