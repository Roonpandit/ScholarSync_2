...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      if (sidebarOpen && !event.target.closest('.sidebar') && !event.target.closest('.toggle-sidebar')) {
        setSidebarOpen(false)
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* Header */}
      <header className={`bg-white fixed w-full top-0 z-50 transition-all duration-200 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-700 hidden sm:block">Masai Attendance</h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative user-dropdown">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg overflow-hidden z-10 border border-gray-100">
      ...
      <div className="flex flex-1 pt-16">
        ...
        {/* Main Content */}
        <main className={`flex-1 transition-all duration-300 pt-6 px-4 pb-8 md:ml-72`}>
...
const NavItem = ({ to, isActive, icon, children, badge = false }) => {
  return (
    <Link 
      ...
      `}
    >
      <div className={`transition-colors duration-150 ${isActive ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-500'}`}>
        {icon}
      </div>
      <span>{children}</span>
      {isActive && <div className="ml-auto w-1 h-5 bg-blue-600 rounded-full"></div>}
      {badge && !isActive && (
        <span className="ml-auto px-1.5 py-0.5 bg-green-100 text-green-800 text-xs font-medium rounded-full">New</span>
      )}
    </Link>
  )
}
