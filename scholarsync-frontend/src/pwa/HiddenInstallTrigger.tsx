...
export function HiddenInstallTrigger() {
  const [popup, setPopup] = useState<PopupState>({ show: false, type: '', message: '' });
  ...
  useEffect(() => {
    const handleInstallResult = (e: Event) => {
      const { outcome, message } = (e as CustomEvent).detail;
...
export function InstallHint({ position }: InstallHintProps) {
