...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch students count (both roles)
      const studentsPromise = get<ApiResponse<unknown[]> & { count?: number }>(API_ENDPOINTS.STUDENTS);
      // Admin also fetches teachers
      const teachersPromise = isAdmin
        ? get<ApiResponse<unknown[]> & { count?: number }>(API_ENDPOINTS.TEACHERS)
        : Promise.resolve(null);

      const [studentsRes, teachersRes] = await Promise.all([studentsPromise, teachersPromise]);

      // Fetch all slots and filter active ones on the client side
      const slotsRes = await get<ApiResponse<SlotData[]>>(API_ENDPOINTS.ATTENDANCE_SLOTS.LIST);
      const now = new Date();
      const allSlots = slotsRes?.data || [];

      const activeSlots = allSlots.filter((slot: SlotData) => {
      ...
      const attendancePromises = activeSlots.map((slot: SlotData) =>
        get<ApiResponse<AttendanceRecord[]>>(API_ENDPOINTS.ATTENDANCE.LIST, { slotId: slot.id })
          .catch(() => ({ data: [] as AttendanceRecord[] }) as ApiResponse<AttendanceRecord[]>)
      );
      const attendanceResponses = await Promise.all(attendancePromises);

      const allAttendance: AttendanceRecord[] = attendanceResponses.flatMap(res => {
        if (!res?.data) return [];
        return (res.data as AttendanceRecord[]).map((record: AttendanceRecord) => ({
          ...record,
      ...
      const recentAtt = allAttendance
        ...
        .sort((a, b) => new Date(b.markedAt || b.markingTime || '').getTime() - new Date(a.markedAt || a.markingTime || '').getTime())
        .slice(0, 5);

      const totalAttendance = allAttendance.length;

      const absentRes = await get<ApiResponse<unknown[]> & { count?: number }>(API_ENDPOINTS.ATTENDANCE.ABSENT_LIST);

      setStats({
        totalStudents: (studentsRes as ApiResponse & { count?: number }).count || 0,
        totalTeachers: isAdmin ? ((teachersRes as ApiResponse & { count?: number })?.count || 0) : 0,
        todayAttendance: totalAttendance,
        activeSlots: activeSlots.length,
        absentStudents: (absentRes as ApiResponse & { count?: number }).count || 0
      });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">{roleLabel} Dashboard</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          ...
          linkTo={ROUTE_CONSTANTS.STUDENTS}
          linkText="View All"
          icon={
        ...
        {isAdmin && (
          <StatCard
            ...
            linkTo={ROUTE_CONSTANTS.TEACHERS}
            linkText="View All"
            icon={
        ...
        <StatCard
          ...
          linkTo={ROUTE_CONSTANTS.ATTENDANCE_SLOTS}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-md lg:col-span-2">
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            ...
            <Link
              to={ROUTE_CONSTANTS.ATTENDANCE_SLOTS}
        ...
        <div className="bg-white rounded-xl shadow-md">
          ...
          <div className="p-6 space-y-4">
            ...
            />
            <ActionButton
              to={ROUTE_CONSTANTS.ATTENDANCE_STATS}
            ...
            {isAdmin && (
              <ActionButton
                to={ROUTE_CONSTANTS.LECTURE_MANAGEMENT}
            ...
            <ActionButton
              to={ROUTE_CONSTANTS.LEAVE_MANAGEMENT}
...
// Stat Card Component
const StatCard = ({ title, value, linkTo, linkText, icon, color }: StatCardProps) => {
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden">
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className={`p-3 rounded-lg ${classes.light}`}>{icon}</div>
        </div>
        <div>
          <h3 className="text-gray-500 text-sm font-medium">{title}</h3>
          <p className="text-3xl font-bold text-gray-800 my-1">{value}</p>
          <Link to={linkTo} className={`inline-flex items-center text-sm ${classes.text} font-medium`}>
            {linkText}
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
...
const ActionButton = ({ to, icon, title, description, color }: ActionButtonProps) => {
  const colorClasses: Record<string, Record<string, string>> = {
    ...
    red: { bg: 'bg-red-50', text: 'text-red-700', icon: 'text-red-600', hover: 'hover:bg-red-100' },
  ...
  return (
    <Link to={to} className={`flex items-center p-4 rounded-lg ${classes.bg} ${classes.hover} transition-colors`}>
