...
import type { Shift, SlotStatus } from '@/types/attendance.types';
...
interface AttendanceResponse {
  ...
  data: Array<{
    id: string;
    student: { id: string; studentCode?: string } | string;
    isPresent?: boolean;
    status: string;
    markedAt?: string;
    location?: { type: string; coordinates: number[] };
...
const AttendanceSlot = () => {
  ...
  const getReviewStatus = async (slotId: string): Promise<ReviewStatus> => {
    try {
      const response = await get<AttendanceResponse>(API_ENDPOINTS.ATTENDANCE.LIST, { slotId });
        ...
        if (student['Attendance Status'] === 'Absent') {
          ...
          headers.forEach((header, colIndex) => {
            ...
            if (!worksheet[cellAddress]) {
              worksheet[cellAddress] = { v: student[header as keyof typeof student] };
            }
  ...
  const isSlotExpired = useCallback((slot: SlotData): boolean => {
  ...
  const fetchSlots = async () => {
    try {
      ...
      const res = await get<SlotsResponse>(API_ENDPOINTS.ATTENDANCE_SLOTS.LIST);
      ...
      const processedSlots: SlotData[] = (res.data || []).map((slot) => {
        ...
        return {
          ...
          formattedDate: startTimeIST ? formatDateDisplay(startTimeIST) : '',
          formattedTime: startTimeIST && endTimeIST ? `${formatTime24h(startTimeIST)} - ${formatTime24h(endTimeIST)}` : '',
        };
      });

      processedSlots.sort((a, b) => {
        const aIST = convertToIST(new Date(b.startTime));
        const bIST = convertToIST(new Date(a.startTime));
        return (aIST?.getTime() || 0) - (bIST?.getTime() || 0);
      });
      ...
      await Promise.all(
        closedSlots.map(async (slot) => {
          const status = await getReviewStatus(slot.id);
          reviewStatusMap[slot.id] = status;
        })
      );
      ...
      toast.error(err.response?.data?.message || 'Failed to fetch attendance slots');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMarkAttendance = async (studentId: string, isPresent: boolean) => {
    try {
      const res = await post<ApiActionResponse>(API_ENDPOINTS.ATTENDANCE.MARK, {
        studentId,
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const preparedTimes = prepareSlotTimes(formData.date, startTime, endTime);

      const res = await post<ApiActionResponse>(API_ENDPOINTS.ATTENDANCE_SLOTS.CREATE, {
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || 'Failed to create attendance slot');
    } finally {
  ...
  const handleDeleteSlot = async (slotId: string) => {
    try {
      ...
      await del<ApiActionResponse>(API_ENDPOINTS.ATTENDANCE_SLOTS.DELETE(slotId));
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || 'Failed to delete attendance slot');
    } finally {
  ...
  const handleViewAttendance = async (slot: SlotData) => {
    try {
      ...
      try {
        ...
        const studentsRes = await get<StudentsResponse>(API_ENDPOINTS.STUDENTS);
        const allStudents: StudentData[] = studentsRes?.data || [];

        // Get attendance for this slot
        const attendanceRes = await get<AttendanceResponse>(API_ENDPOINTS.ATTENDANCE.LIST, { slotId: slot.id });
        ...
        attendanceData.forEach((record) => {
          if (record.student) {
            ...
            attendanceMap[studentId] = {
              ...
              photo: record.photo,
              studentCode: record.studentCode || (typeof record.student !== 'string' ? record.student.studentCode || '' : ''),
            };
          }
        });

        // Process all students - only show those with attendance records
        const processedStudents: StudentData[] = allStudents
  ...
  const handleApproveAttendance = async (attendanceId: string) => {
    try {
      await post<ApiActionResponse>(API_ENDPOINTS.ATTENDANCE.STATUS(attendanceId), { action: 'approve' });
  ...
  const handleRejectAttendance = async (attendanceId: string) => {
    ...
    try {
      await post<ApiActionResponse>(API_ENDPOINTS.ATTENDANCE.STATUS(attendanceId), { action: 'reject', remark: remark.trim() });
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {slots.length > 0 ? `Showing ${slots.length} attendance slot${slots.length !== 1 ? 's' : ''}` : 'No attendance slots found'}
          </p>
        </div>
        ...
        {slots.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {filterDate ? `No slots found for ${new Date(filterDate).toLocaleDateString()}` : 'Create a new attendance slot to get started.'}
            </p>
            {filterDate && (
              <button
          ...
          <>
            ...
            <div className="mb-6 border-b border-gray-200">
              <nav className="-mb-px flex space-x-8">
                ...
                >
                  Review Pending ({slots.filter((s) => s.status === 'closed' && slotReviewStatus[s.id]?.hasPendingReviews).length})
                </button>
                <button
                ...
                >
                  Review Done ({slots.filter((s) => s.status === 'closed' && !slotReviewStatus[s.id]?.hasPendingReviews).length})
                </button>
              </nav>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-300">
                    <thead className="bg-gray-50 sticky top-0 z-10">
                      <tr>
                        ...
                        <th scope="col" className="px-3 py-3.5 text-right-17 text-xs sm:text-sm font-semibold text-gray-900">
                          Action
                        </th>
                      </tr>
                    </thead>
                      ...
                      {(() => {
                        const filteredSlots = slots.filter((slot) => {
                          ...
                          if (reviewFilter === 'pending') {
                            return slot.status === 'closed' && slotReviewStatus[slot.id]?.hasPendingReviews;
                          }
                        ...
                        return filteredSlots.map((slot) => (
                          <tr key={slot.id} className={`hover:bg-gray-50 ${slot.isExpired ? 'opacity-70' : ''}`}>
                              ...
                              <div className="flex items-center">
                                ...
                                <div className="ml-2 sm:ml-4">
                                  ...
                                  <div className="text-gray-500 sm:hidden mt-1">
                                    <span className="text-xs font-medium text-gray-700">{slot.lecture?.name || 'N/A'}</span>
                                  </div>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="whitespace-nowrap px-3 py-4 text-xs sm:text-sm">
                              <div className="flex items-center">
                                {getStatusBadge(slot)}
                                {getReviewBadge(slot, slotReviewStatus[slot.id]?.pendingCount || 0)}
                              </div>
                            </td>
      ...
      <Modal
        ...
        title={`Attendance for ${currentSlot ? formatDateDisplay(currentSlot.date) : ''} - ${
              ...
              <>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 space-y-2 sm:space-y-0 sticky top-0 bg-white pt-2 pb-2 z-10">
                  <div className="text-sm text-gray-500">
                    ...
                    {new Date(currentSlot.startTime).toLocaleTimeString('en-US', {
                      hour: 'numeric',
                      minute: 'numeric',
                      hour12: false,
                      timeZone: 'Asia/Kolkata',
                ...
                <div className="mb-4 flex space-x-2 bg-gray-100 p-1 rounded-lg">
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${
                      attendanceFilter === 'all' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                    }`}
                  >
                    All ({students.length})
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${
                      attendanceFilter === 'needs_review' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                    }`}
                  >
                    Needs Review ({students.filter((s) => attendance[s.id]?.status === 'awaiting_approval').length})
                  </button>
                  <button
                    onClick={() => setAttendanceFilter('reviewed')}
                    className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${
                      attendanceFilter === 'reviewed' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                    }`}
                  >
                    Reviewed (
                    {
                                ...
                                return (
                                  <tr key={student.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-2 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                      ...
                                      <div className="text-xs sm:text-sm text-gray-500 hidden sm:block">{student.email}</div>
                                    </td>
                                    <td className="px-2 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                      {(() => {
                                    ...
                                    <td className="px-2 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                                      {studentAttendance?.status === 'awaiting_approval' && (
                                        <div className="flex gap-2">
                                          <button
                                            onClick={() => handleApproveAttendance(studentAttendance.id)}
                                          ...
                                          <button
                                            onClick={() => handleRejectAttendance(studentAttendance.id)}
        ...
        {photoModalOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg max-w-md w-11/12 sm:w-96 max-h-[80vh] overflow-auto">
              ...
              <div className="p-3">
                {selectedPhoto ? (
                  <img src={selectedPhoto} alt="Student" className="w-full max-h-[60vh] object-contain rounded-lg" />
                ) : (
      ...
      <Modal isOpen={showAddForm} onClose={() => !creatingSlot && setShowAddForm(false)} title="Create New Attendance Slot" size="lg">
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm ${
                    creatingSlot ? 'bg-gray-100 cursor-not-allowed' : ''
                  }`}
                  required
                />
              </div>
            ...
            {/* Lecture Selection */}
            <div className="mt-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Select Lectures *</label>
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => {
                          setFormData((prev) => ({
                            ...prev,
                            lectures: prev.lectures.filter((id) => id !== lecture.id),
                          }));
                        }}
              ...
              >
                ...
                {lectures
                  .filter((lecture) => !formData.lectures.includes(lecture.id))
                  .map((lecture) => (
            ...
            <div className="flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6">
              <button
                ...
                disabled={creatingSlot}
                className={`w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${
                  creatingSlot ? 'text-gray-400 bg-gray-100 cursor-not-allowed' : 'text-gray-700 bg-white hover:bg-gray-50'
                }`}
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={creatingSlot}
                className={`w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 ${
                  creatingSlot ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 cursor-pointer'
                }`}
              >
                {creatingSlot ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500 mt-1">
                This action cannot be undone. All attendance records for this slot will be permanently removed.
              </p>
              {slotToDelete && (
                <div className="mt-3 p-3 bg-gray-50 rounded-md">
                  <div className="text-sm text-gray-700">
                    <strong>Date:</strong>{' '}
                    {new Date(slotToDelete.date).toLocaleDateString('en-US', {
                      weekday: 'long',
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                  ...
                  <div className="text-sm text-gray-700">
                    <strong>Shift:</strong> {slotToDelete.shift.charAt(0).toUpperCase() + slotToDelete.shift.slice(1)}
                  </div>
                  <div className="text-sm text-gray-700">
                    <strong>Time:</strong>{' '}
                    {new Date(slotToDelete.startTime).toLocaleTimeString('en-US', {
                      hour: 'numeric',
                      minute: 'numeric',
                      hour12: false,
                      timeZone: 'Asia/Kolkata',
                    ...
                    -{' '}
                    {new Date(slotToDelete.endTime).toLocaleTimeString('en-US', {
                      hour: 'numeric',
                      minute: 'numeric',
                      hour12: false,
                      timeZone: 'Asia/Kolkata',
          ...
          <div className="flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6">
            <button
              ...
              className={`w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 ${
                deletingSlot ? 'text-gray-400 bg-gray-100 cursor-not-allowed' : 'text-gray-700 bg-white hover:bg-gray-50'
              }`}
            >
              Cancel
            </button>
            <button
              ...
              disabled={deletingSlot}
              className={`w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all duration-200 ${
                deletingSlot ? 'bg-red-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700 cursor-pointer'
              }`}
            >
              {deletingSlot ? (
                <div className="inline-flex items-center">
