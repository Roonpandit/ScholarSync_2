...
const BulkUploads = () => {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      reader.onload = (evt) => {
        ...
        const headers = (jsonData[0] as string[]).map((header: string) => header.trim().toLowerCase());

        // Validate headers
        const requiredHeaders = ['name', 'email', 'studentcode', 'phone'];
        const isValid = requiredHeaders.every(header =>
          headers.some((h: string) => h.toLowerCase() === header)
        );
        ...
        const dataObjects: PreviewRow[] = jsonData.slice(1).map((row: unknown) => {
          const r = row as string[];
          return {
            name: r[headerMap['name']]?.toString()?.trim() || '',
            email: r[headerMap['email']]?.toString()?.trim() || '',
            studentcode: r[headerMap['studentcode']]?.toString()?.trim() || '',
            phone: r[headerMap['phone']]?.toString()?.trim() || ''
          };
        });
        ...
        const previewRows = formattedData.map(row => ({
          ...row,
          errors: validateRow(row)
        }));

        // Get all validation errors
        const validationErrors = previewRows.map((row, index) => {
        ...
        if (validationErrors.length > 0) {
          const errorMessages = validationErrors.map((v) =>
            `Row ${v!.rowNumber}: ${v!.errors.join(', ')}`
          );
        ...
        const individualValidationErrors = formattedData.map((row) => {
          ...
          if (!row.email.trim()) errs.email = 'Email is required';
          if (!row.email.trim() || !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(row.email))
            errs.email = 'Invalid email address';
          if (!row.studentcode.trim()) errs.studentcode = 'Student code is required';
          if (!row.phone.trim()) errs.phone = 'Phone number is required';
        ...
        const invalidRows = individualValidationErrors.filter(errs => Object.keys(errs).length > 0);
  ...
  const validateRow = (row: PreviewRow): Record<string, string> => {
    ...
    if (!email.trim() || !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(email))
      errs.email = 'Invalid email address';
    if (!studentCode.trim()) errs.studentcode = 'Student code is required';
  ...
  const handleUpload = async () => {
    try {
      ...
      const CHUNK_SIZE = 10;
      const chunks: typeof formattedStudents[] = [];
      ...
      // Upload all chunks
      const allResponses: Array<BulkUploadResponse | { error: unknown }> = [];
      for (let i = 0; i < chunks.length; i++) {
        const chunk = chunks[i];

        try {
          const response = await post<BulkUploadResponse>(
            API_ENDPOINTS.BULK_REGISTER,
            chunk,
      ...
      const successResponses = allResponses.filter(r => !('error' in r) && (r as BulkUploadResponse).success);
      const errorResponses = allResponses.filter(r => ('error' in r) || !(r as BulkUploadResponse).success);

      if (successResponses.length > 0) {
        toast.success((successResponses[successResponses.length - 1] as BulkUploadResponse).message);
      }

      if (errorResponses.length > 0) {
        const firstError = errorResponses[0];
        if ('error' in firstError) {
          const err = firstError.error as { response?: { data?: { message?: string } } };
          toast.error(err.response?.data?.message || 'Failed to upload students');
        } else {
          toast.error((firstError as BulkUploadResponse).message || 'Failed to create students');
        }
      }
