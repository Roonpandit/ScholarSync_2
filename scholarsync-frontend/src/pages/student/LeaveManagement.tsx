...
import type { LeaveRequest, LeaveType, LeaveStatus } from '@/types/leave.types';
import type { Lecture } from '@/types/lecture.types';

interface LectureTeacherPair {
  lectureId: string;
  teacherId: string;
}
...
interface LeaveRequestWithPopulated extends Omit<LeaveRequest, 'lectureId' | 'teacherId'> {
  lectureId: { id: string; name: string } | null;
  teacherId: { id: string; name: string } | null;
}
...
const LeaveManagement: React.FC = () => {
  ...
  const [lectureTeacherPairs, setLectureTeacherPairs] = useState<LectureTeacherPair[]>([{ lectureId: '', teacherId: '' }]);
  const [availableLectures, setAvailableLectures] = useState<Lecture[]>([]);
  const [teachersForLecture, setTeachersForLecture] = useState<Record<string, TeacherInfo>>({});
  ...
  const [leaveRequests, setLeaveRequests] = useState<LeaveRequestWithPopulated[]>([]);
  const [filteredRequests, setFilteredRequests] = useState<LeaveRequestWithPopulated[]>([]);
  ...
  const [selectedRequest, setSelectedRequest] = useState<LeaveRequestWithPopulated | null>(null);
  const [showDetailModal, setShowDetailModal] = useState<boolean>(false);
  const [showCancelModal, setShowCancelModal] = useState<boolean>(false);
  const [showResendModal, setShowResendModal] = useState<boolean>(false);
  const [showDeleteModal, setShowDeleteModal] = useState<boolean>(false);
  const [cancelReason, setCancelReason] = useState<string>('');
  const [resendRemark, setResendRemark] = useState<string>('');
  const [deleteLoading, setDeleteLoading] = useState<boolean>(false);

  // Fetch lectures on mount
  useEffect(() => {
    fetchNondefaultLectures();
  }, []);
  ...
  const fetchNondefaultLectures = async (): Promise<void> => {
    try {
      const response = await get<ApiResponse<Lecture[]>>(API_ENDPOINTS.LECTURES.LIST, { filter: 'non-default' });
      setAvailableLectures(response?.data || []);
    } catch (error) {
  ...
  const fetchTeacherForLecture = async (lectureId: string, index: number): Promise<void> => {
    try {
      const response = await get<ApiResponse<TeacherInfo>>(API_ENDPOINTS.LECTURES.TEACHER(lectureId));
      const teacherData = response?.data;

      if (teacherData) {
        setTeachersForLecture((prev) => ({
          ...prev,
          [lectureId]: teacherData
        }));

        // Auto-select the teacher
        const newPairs = [...lectureTeacherPairs];
        newPairs[index].teacherId = teacherData.id;
  ...
  const handleLectureChange = (index: number, lectureId: string): void => {
    const newPairs = [...lectureTeacherPairs];
  ...
  const handleApplyLeave = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    ...
    setLoading(true);
    try {
      await post(API_ENDPOINTS.LEAVE.APPLY, {
  ...
  const fetchLeaveRequests = async (): Promise<void> => {
    try {
      const params: Record<string, unknown> = {};
      if (filters.lecture.length > 0) params.lecture = filters.lecture;
      if (filters.teacher.length > 0) params.teacher = filters.teacher;
      if (filters.status.length > 0) params.status = filters.status;
      if (filters.fromDate) params.fromDate = filters.fromDate;
      if (filters.toDate) params.toDate = filters.toDate;

      const response = await get<ApiResponse<LeaveRequestWithPopulated[]>>(API_ENDPOINTS.LEAVE.MY_REQUESTS, params);
  ...
  const handleDeleteRequest = async (): Promise<void> => {
    ...
    try {
      await del(API_ENDPOINTS.LEAVE.DELETE(selectedRequest.id));
      ...
      const axiosError = error as { response?: { data?: { message?: string } } };
      toast.error(axiosError.response?.data?.message || 'Failed to delete request');
    } finally {
  ...
  const handleCancelRequest = async (): Promise<void> => {
    ...
    try {
      await post(API_ENDPOINTS.LEAVE.MANAGE(selectedRequest.id), { action: 'cancel', cancelReason });
      ...
      const axiosError = error as { response?: { data?: { message?: string } } };
      toast.error(axiosError.response?.data?.message || 'Failed to cancel request');
    }
  };

  const handleResendRequest = async (): Promise<void> => {
    if (!selectedRequest) return;
    ...
    try {
      await post(API_ENDPOINTS.LEAVE.MANAGE(selectedRequest.id), { action: 'resend', studentRemark: resendRemark });
      ...
      const axiosError = error as { response?: { data?: { message?: string } } };
      toast.error(axiosError.response?.data?.message || 'Failed to resend request');
    }
  };
  ...
  const canResend = (request: LeaveRequestWithPopulated): boolean => {
                      ...
                      <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleLectureChange(index, e.target.value)}
                      ...
                      <div className="flex items-center mb-2 sm:mb-0">
                        ...
                        <div className="ml-3">
                          ...
                          <p className="text-sm text-gray-500">
                            {formatDateLocal(request.fromDate)} - {formatDateLocal(request.toDate)}
                          </p>
                        </div>
                      </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatDateLocal(selectedRequest.fromDate)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">To Date</p>
                  <p className="font-medium">{formatDateLocal(selectedRequest.toDate)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{formatDateLocal(selectedRequest.appliedAt)}</p>
                </div>
                {selectedRequest.approvedAt && (
                  <div>
                    <p className="text-sm text-gray-500">Approved On</p>
                    <p className="font-medium">{formatDateLocal(selectedRequest.approvedAt)}</p>
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full">
            ...
            <div className="p-6">
              ...
              <textarea
                value={resendRemark}
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setResendRemark(e.target.value)}
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
