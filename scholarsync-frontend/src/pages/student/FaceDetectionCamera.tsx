...
interface RealtimeValidation {
  type: 'no_face' | 'multiple_faces' | 'single_face' | 'invalid_face' | 'error';
  isValid: boolean;
  message: string;
  color: string;
  confidence?: number;
...
interface BlazeFaceModel {
  estimateFaces: (input: HTMLVideoElement, returnTensors: boolean) => Promise<BlazeFacePrediction[]>;
}
 ...
 * - onCapture: (canvas) => void - Callback when photo is captured
 * - onCancel: () => void - Callback when camera is cancelled
 * - videoRef: ref - Reference to pass to parent for video element
 */
const FaceDetectionCamera: React.FC<FaceDetectionCameraProps> = ({ show, onCapture, onCancel, videoRef }) => {
  const [detectorModel, setDetectorModel] = useState<BlazeFaceModel | null>(null);
  const [modelReady, setModelReady] = useState<boolean>(false);
  const [modelLoading, setModelLoading] = useState<boolean>(false);
  const [realtimeValidation, setRealtimeValidation] = useState<RealtimeValidation | null>(null);
  ...
  useEffect(() => {
    const loadModel = async () => {
      ...
      try {
        ...
        setDetectorModel(model as unknown as BlazeFaceModel);
        setModelReady(true);
  ...
  const checkLandmarksVisibility = (face: BlazeFacePrediction, canvas: HTMLCanvasElement): LandmarkCheck => {
  ...
  const calculateFaceMetrics = (face: BlazeFacePrediction, videoWidth: number, videoHeight: number): FaceMetrics => {
  ...
  const validateFace = (face: BlazeFacePrediction, videoWidth: number, videoHeight: number, canvas: HTMLCanvasElement): FaceValidation => {
  ...
  const detectFaces = async (videoElement: HTMLVideoElement, canvas: HTMLCanvasElement): Promise<ValidatedFace[]> => {
    if (!detectorModel) {
    ...
    try {
      ...
      const validatedFaces: ValidatedFace[] = predictions.map((face) => ({
        face,
  ...
  const handleCapture = (): void => {
    if (canCapture && onCapture && canvasRef.current) {
