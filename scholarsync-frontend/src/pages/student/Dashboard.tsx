...
import type { AttendanceSlot, AttendanceRecord } from '@/types/attendance.types';
...
const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch all slots
        const slotsRes = await get<ApiResponse<AttendanceSlot[]>>(API_ENDPOINTS.STUDENT_ATTENDANCE.ACTIVE_SLOTS);
        ...
        const attendanceRes = await get<ApiResponse<AttendanceRecord[]>>(
        ...
        const absenceRes = await get<AbsenceResponse>(
          API_ENDPOINTS.STUDENT_ATTENDANCE.HISTORY,
          { type: 'absent', month: currentMonth, year: currentYear }
        );
        ...
        const totalAbsences = absenceRes?.totalAbsences || 0;
        const totalPending = absenceRes?.totalPending || 0;
  ...
  const calculateAttendancePercentage = (stats: AttendanceStats): number => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow hover:shadow-md transition-shadow p-6 border-b-4 border-orange-500">
          ...
          <div className="mt-4">
            <Link
              to="/student/attendance?type=absent"
                ...
                {slots.filter((slot) => slot.status === 'active' || slot.status === 'upcoming').map((slot) => (
                  ...
                  >
                    ...
                    <div className="mt-4">
                      {slot.status === 'active' ? (
                        <Link
                          to={`/student/mark-attendance?slotId=${slot.id}`}
