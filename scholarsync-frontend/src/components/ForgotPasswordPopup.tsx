...
const ForgotPasswordPopup: React.FC<ForgotPasswordPopupProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const checkResponse = await get<ApiResponse<CheckEmailResponse>>(
        `${API_ENDPOINTS.AUTH.LOGIN.replace('/login', '/check-email')}?email=${encodeURIComponent(email)}`
      );

      if (checkResponse?.data?.exists) {
        // Email exists, proceed with password reset
        const response = await post<ApiResponse>(API_ENDPOINTS.AUTH.FORGOT_PASSWORD, { email });
        setIsEmailSent(true);
              ...
              <div className="flex flex-col space-y-3 mb-6">
                <button
                  onClick={async () => {
                    ...
                    setIsResending(true);
                    try {
                      await post<ApiResponse>(API_ENDPOINTS.AUTH.FORGOT_PASSWORD, { email });
