...
import wfmsRoutes from './api/routes/routes.js';
...
app.use(initiateMiddleware({ service: 'WFMS' }));

app.get('/', (req, res) => {
  res.json({ success: true, message: 'ScholarSync WFMS Service is running' });
});

app.use(ROUTES.BASE_ROUTE + ROUTES.SWAGGER_ROUTE, swaggerUi.serve, swaggerUi.setup(swaggerDocument, { explorer: true }));
app.use(ROUTES.BASE_ROUTE, wfmsRoutes);
