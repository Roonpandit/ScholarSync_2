...
import iamRoutes from './api/routes/routes.js';
...
app.use(initiateMiddleware({ service: 'IAM' }));

app.get('/', (req, res) => {
  res.json({ success: true, message: 'ScholarSync IAM Service is running' });
});

app.use(ROUTES.BASE_ROUTE + ROUTES.SWAGGER_ROUTE, swaggerUi.serve, swaggerUi.setup(swaggerDocument, { explorer: true }));
app.use(ROUTES.BASE_ROUTE, iamRoutes);
