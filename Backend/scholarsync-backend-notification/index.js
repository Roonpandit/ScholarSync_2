...
import notificationRoutes from './api/routes/routes.js';
...
app.use(initiateMiddleware({ service: 'NOTIFICATION' }));

app.get('/', (req, res) => {
  res.json({ success: true, message: 'ScholarSync Notification Service is running' });
});

app.use(ROUTES.BASE_ROUTE + ROUTES.SWAGGER_ROUTE, swaggerUi.serve, swaggerUi.setup(swaggerDocument, { explorer: true }));
app.use(ROUTES.BASE_ROUTE, notificationRoutes);
